...
include realpath($_SERVER['DOCUMENT_ROOT'] . "/../scripts/utils.php");
...
{
    $url = "https://test.cedmod.nl/api/getbanlogs.php?alias=".GetAlias()."&limit=unlimited";
    $ch = curl_init();
...
if (!empty($options->Fetch))
{
    if (empty($options->Limit))
        ...
        $url = "https://test.cedmod.nl/api/getbanlogs.php?alias=".GetAlias()."&limit=".$options->Limit;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL,$url);
    curl_setopt($ch, CURLOPT_USERAGENT, "CedMod Community management webpanel" . $domain);
    curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
    ...
    if ($bans["success"] == "true")
    {
        ...
        userid = ? AND server LIKE '".GetAlias()."' LIMIT 1"
                ...
                if ($query1->rowCount() == 1) {
                    $user = $query1->fetch();
                    $ip = $user['ip'];
                    $query = $db->prepare("UPDATE " . GetAlias() . "ban_logs_all SET ip=:ip WHERE playersteamid=:steamid64 ORDER BY id DESC");
            ...
            if (empty($ban["username"])) {
                ...
                if (empty($username))
                    $username = "Unable to resolve username.";
                $db = createPDO(realpath($_SERVER['DOCUMENT_ROOT'] . "/../config/mysqlban.ini"));
                $query = $db->prepare("UPDATE " . GetAlias() . "ban_logs_all SET username=:username WHERE playersteamid=:steamid64 ORDER BY id DESC");
                $query->bindParam(":steamid64", $ban['playersteamid']);
                $query->bindParam(":username", $username);
                $query->execute();
                $ban["username"] = $username;
            }
            echo generateBanLogCard($ban);
        }
    }
    else
        echo $bans["message"];
    ...
    $url = "https://test.cedmod.nl/api/getbanlog.php?alias=".GetAlias()."&id=".$options->steamID."&name=".$options->nickname."&aname=".$options->modname;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL,$url);
    curl_setopt($ch, CURLOPT_USERAGENT, "CedMod Community management webpanel" . $domain);
    curl_setopt($ch, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
